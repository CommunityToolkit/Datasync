...
> [!NOTE]
> Sqlite stores DateTimeOffset using a second accuracy by default. The Datasync Community Toolkit does not rely on the storage of the `UpdatedAt` field using date/time conversions.  Consider using [a ValueConverter](https://learn.microsoft.com/ef/core/modeling/value-conversions?tabs=data-annotations) to store date/time values other than the `UpdatedAt` appropriately.
...
>
> If you decide to use the same entity type for both service and client, ensure you use repository updating of the metadata properties.
> Do **NOT** allow the database to do automatic updates of the entity properties on the client side.
