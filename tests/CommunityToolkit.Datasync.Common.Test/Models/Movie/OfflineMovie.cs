...
public class OfflineMovie : OfflineClientEntity, IMovie, IEquatable<IMovie>
{
    public OfflineMovie() { }
    public OfflineMovie(object source)
    {
        if (source is ITableData metadata)
        {
            Id = metadata.Id;
            Deleted = metadata.Deleted;
            UpdatedAt = metadata.UpdatedAt ?? DateTimeOffset.MinValue;
