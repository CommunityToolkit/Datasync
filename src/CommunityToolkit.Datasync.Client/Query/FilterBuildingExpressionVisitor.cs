...
// Long switch statements are easier to read than switch expressions with pattern matching.
...
/// <summary>
/// Walks the LINQ expression tree to compile the expression into a simplified tree of nodes that can be used for OData or SQL production.
/// </summary>
internal class FilterBuildingExpressionVisitor
{
    ...
    private static readonly MethodInfo arrayContainsMethod = typeof(Enumerable).GetRuntimeMethods()
...
#pragma warning restore CS8600 // Converting null literal or possible null value to non-nullable type.
    ...
    /// </summary>
    /// <param name="serializerOptions">The <see cref="JsonSerializerOptions"/> to use for resolving names.</param>
    internal FilterBuildingExpressionVisitor(JsonSerializerOptions serializerOptions) : base()
    {
    ...
    internal static QueryNode? Compile(Expression expression, JsonSerializerOptions serializerOptions)
    {
        FilterBuildingExpressionVisitor visitor = new(serializerOptions);
        _ = visitor.Visit(expression);
        return visitor.FilterExpression.FirstOrDefault();
    }

    /// <summary>
    /// Converts the name provided according to the property naming policy.  If a property naming policy
    /// is not set, then the name is returned.
    /// </summary>
    /// <param name="name">The name to be converted.</param>
    /// <param name="serializerOptions">The serializer options to use for converting the name.</param>
    /// <returns>The converted name.</returns>
    internal static string ConvertName(string name, JsonSerializerOptions serializerOptions)
    {
        return serializerOptions.PropertyNamingPolicy?.ConvertName(name) ?? name;
    }
    ...
    internal static string GetTableMemberName(Expression expression, JsonSerializerOptions serializerOptions)
    {
        if (expression is MemberExpression member && member.Expression != null)
        {
            if (member.Expression.NodeType == ExpressionType.Parameter || (member.Expression.NodeType == ExpressionType.Convert && member.Member.MemberType == MemberTypes.Property))
            {
                return ConvertName(member.Member.Name, serializerOptions);
            }
        }
    ...
    {
        if (IsEnumExpression(node, out UnaryExpression? enumExpression, out ConstantExpression? constant))
        {
    ...
    private MemberExpression VisitMemberExpression(MemberExpression node)
    {
        ...
        string memberName = GetTableMemberName(node, SerializerOptions);
    ...
    internal bool IsEnumExpression(BinaryExpression node, [NotNullWhen(true)] out UnaryExpression? unaryExpression, [NotNullWhen(true)] out ConstantExpression? constantExpression)
    {
